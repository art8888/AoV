...
class aLang {
	...
	function parse() {
		...
		if (!file_exists($filename)) {
			printf('Error: Language not se yet! (%s don"t exist)', htmlentities($this->lang), htmlentities($filename));
			exit;
		}
	...
	if (!$this->exists($this->section, $varname)) {
		return sprintf('Transale for %s(%s) not available!', htmlentities($this->section), htmlentities($varname));
		}
	...
	function grab($section, $varname) {
		if (!$this->exists($section, $varname)) {
			return sprintf('Translate for %s[%s] not available!', htmlentities($section), htmlentities($varname));
		}
